...
final class CloudNPCModule extends Module {
    ...
    public function __construct() {
        parent::__construct(InGameModuleCache::NPC_MODULE, "Activate server npcs for transportation");
    }
    ...
    private function load(): void {
        foreach ($this->getGroupsConfig()->getAll() as $groupId => $groupData) {
            if (($templateGroup = TemplateGroup::read($groupData)) !== null && $groupId == $groupData["id"]) {
                $this->templateGroups[$groupId] = $templateGroup;
            }
        }

        foreach ($this->getModelsConfig()->getAll() as $modelId => $modelData) {
            if (($model = CustomSkinModel::read($modelData)) !== null && $modelId == $modelData["id"]) {
    ...
    public function addCloudNPC(CloudNPC $npc): bool {
        ...
        try {
            ($cfg = $this->getNPCConfig())->set($positionString, $npc->write());
            $cfg->save();
    ...
    public function addTemplateGroup(TemplateGroup $templateGroup): bool {
        ...
        try {
            ($cfg = $this->getGroupsConfig())->set($templateGroup->getId(), $templateGroup->write());
            $cfg->save();
        } catch (JsonException $exception) {
            CloudNPCModule::get()->getLogger()->logException($exception);
            return false;
    ...
    public function addSkinModel(CustomSkinModel $model): bool {
        ...
        try {
            ($cfg = $this->getModelsConfig())->set($model->getId(), $model->write());
            $cfg->save();
        } catch (JsonException $exception) {
            CloudNPCModule::get()->getLogger()->logException($exception);
            return false;
    ...
    public function removeTemplateGroup(TemplateGroup $templateGroup): bool {
        ...
        try {
            ...
            foreach (array_filter($this->npcs, fn(CloudNPC $npc) => $npc->hasTemplateGroup() &&
                $npc->getTemplate()->getId() == $templateGroup->getId()) as $npc) {
    ...
    public function removeSkinModel(CustomSkinModel $model): bool {
        ...
        try {
            ...
            foreach (array_filter($this->npcs, fn(CloudNPC $npc) => $npc->getSkinModel() !== null &&
                $npc->getSkinModel()->getId() == $model->getId()) as $npc) {
                $this->removeCloudNPC($npc);
            }
        }
    ...
    public function editTemplateGroup(TemplateGroup $templateGroup): bool {
        try {
            ...
            foreach ($this->getCloudNPCsByGroup($templateGroup) as $npc) $npc->getTemplate()->applyData($templateGroup->write());
        }
    ...
    public function geTemplateGroupByDisplay(string $displayName): ?TemplateGroup {
        return array_find($this->templateGroups, fn(TemplateGroup $group) => $group->getDisplayName() == $displayName);
    }
