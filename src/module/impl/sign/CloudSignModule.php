...
final class CloudSignModule extends Module {
    ...
    public function __construct() {
        parent::__construct(InGameModuleCache::SIGN_MODULE, "Activate server signs for transportation");
    }

    public function onEnable(): void {
        $this->task = new CloudSignTask();
        $this->getServer()->getPluginManager()->registerEvents($this->listener = new SignListener(), $this->getPlugin());
        $this->load();
    }
    ...
    private function load(): void {
        foreach ($this->getCloudSignConfig()->getAll() as $positionString => $cloudSign) {
            if (($cloudSign = CloudSign::read($this->checkForMigration($cloudSign))) !== null) {
    ...
    public function addCloudSign(CloudSign $cloudSign): bool {
        ...
        try {
            ($cfg = $this->getCloudSignConfig())->set($positionString, $cloudSign->write());
            $cfg->save();
